---
title: 'Creamy Tortellini Soup'
description: 'Creamy tortellini soup with Italian sausage, spinach, and basil.'
serves: 6
prepTime: '10 min'
cookTime: '25 min'
activeTimeMinutes: 35
totalTimeMinutes: 35
course:
  - 'Main'
cuisine:
  - 'Italian'
cookingMethod:
  - 'Stovetop'
tags:
  - 'One Pan'
ingredients:
  - title: Soup
  - name: 'Italian sausage'
    quantity: 1
    unit: 'lb'
  - name: 'garlic, minced'
    quantity: 3
    unit: 'clove'
  - name: 'medium onion, diced'
    quantity: 1
    unit: ''
  - name: 'Italian seasoning'
    quantity: 2
    unit: 'tsp'
  - name: 'all-purpose flour'
    quantity: 3
    unit: 'Tbsp'
  - name: 'chicken stock'
    quantity: 4
    unit: 'cup'
  - name: 'frozen cheese tortellini'
    quantity: 9
    unit: 'oz'
  - name: 'spinach'
    quantity: 2
    unit: 'cup'
  - name: 'heavy cream (or 1/2 cup milk + 2 Tbsp butter)'
    quantity: 0.333
    unit: 'cup'
  - name: 'fresh basil, chopped'
    quantity: 0.333
    unit: 'cup'
  - name: 'tomato sauce (or tomato paste + water)'
    quantity: 8
    unit: 'oz'
  - name: 'Parmesan cheese, for serving'
    quantity: 1
    unit: 'to taste'

pubDate: 'Dec 26 2025'
heroImage: './creamy-tortellini-soup.png'
---

<div class="step">

In a large pot over medium heat, cook [[Italian sausage]] with [[medium onion, diced]] until browned. Drain excess fat if needed.

</div>

<div class="step">

Stir in [[garlic, minced]] and [[Italian seasoning]] and cook for 1 minute.

</div>

<div class="step">

Whisk [[all-purpose flour]] into [[heavy cream (or 1/2 cup milk + 2 Tbsp butter)]] until smooth. Add [[chicken stock]] and [[tomato sauce (or tomato paste + water)]], bring to a boil, and stir in the cream mixture.

</div>

<div class="step">

Add [[frozen cheese tortellini]] and cook at a gentle boil for about 15 minutes, or until tender.

</div>

<div class="step">

Stir in [[spinach]] and [[fresh basil, chopped]] and cook until wilted.

</div>

<div class="step">

Serve hot with [[Parmesan cheese, for serving]].

</div>
